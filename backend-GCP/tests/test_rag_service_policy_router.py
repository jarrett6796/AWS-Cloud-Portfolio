...
class RagServicePolicyRouterTest(BaseRagServiceTest):
    ...
    def test_punctuated_greeting_returns_direct_response_without_retrieval(self):
        result = self.rag_service.answer_question(
            "Hi.",
            session_id="session-direct-greeting-punctuation",
        )

        self.assertIn("portfolio architecture", result["answer"])
        self.assertEqual(result["sources"], [])
        self.assertEqual(result["session_id"], "session-direct-greeting-punctuation")
        self.assert_no_retrieval_or_gemini_calls()
    ...
    def test_punctuated_vague_query_returns_clarification_without_retrieval(self):
        result = self.rag_service.answer_question(
            "Tell me more.",
            session_id="session-clarify-punctuation",
        )

        self.assertIn("Can you clarify", result["answer"])
        self.assertEqual(result["sources"], [])
        self.assertEqual(result["retrieval_query"], "Tell me more.")
        self.assertFalse(result["query_rewritten"])
        self.assert_no_retrieval_or_gemini_calls()
